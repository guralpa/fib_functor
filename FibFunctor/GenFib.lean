...
class GenFib (F : ℕ → ℕ) where
  genfib_add_two : F (n + 2) = (F n) + (F (n + 1))

/-- Shows that a GenFib `F` indeed satisfies the Fibonacci recurrence `Fₙ₊₂ = Fₙ + Fₙ₊₁.` -/
theorem genfib_add_two [f : GenFib F] {n : ℕ} : F (n + 2) = F n + F (n + 1) :=
  f.genfib_add_two

lemma genfib_add_one [GenFib F] : ∀ {n}, n ≠ 0 → F (n + 1) = F (n - 1) + F n
  | _n + 1, _ => genfib_add_two

-- theorem genfib_le_fib_succ [GenFib F] {n : ℕ} : F n ≤ F (n + 1) := by cases n <;> simp [genfib_add_two]
